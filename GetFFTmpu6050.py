...
InputSampleRate = input("Sample Rate(32.25Hz to 2000Hz) ? ")  # Accel max_freq is 1kHz
InputSampleNumber = input("Number of sample to take (Max buffer~37) ? ")
...
print("Capturing {0} samples at {1} samples/sec".format(
    TargetSampleNumber, mpu6050.SampleRate))
...
if Total > 0:
    ...
    for loop in range(TargetSampleNumber):
        ...
        FO.write("{0:6.3f}\t{1:6.3f}\t{2:6.3f}\t{3:6.3f}\t".format(
            CurrentForce, I.Gx, I.Gy, I.Gz))
        FO.write("{0:5.1f}\t{1:6.3f}\t{2:6.3f}\t{3:6.3f}\n".format(
            I.Temperature, I.Gyrox, I.Gyroy, I.Gyroz))

    FO.close()
    print("Calculate FFT")

    fourier = np.fft.fft(fftdata)

    print("Save FFTData.txt file")
    FO = open("FFTData.txt", "w")
    fftData = np.abs(fourier[0:int(len(fourier) / 2 + 1)]) / TargetSampleNumber
    frequency = []
    FO.write("Frequency\tFFT\n")
    Peak = 0
    PeakIndex = 0
    for loop in range(int(TargetSampleNumber / 2 + 1)):
